...
from empirical_deep_hedging.testing import test_run

from empirical_deep_hedging.include.actor_critic import ActorCritic
from empirical_deep_hedging.include.utility import (
    StatePrepare,
    get_model_number,
    maybe_make_dirs,
)
from empirical_deep_hedging.include.settings import getSettings, saveSettings, setSettings


def set_random_seeds(seed):
    """Seed the stochastic components used during training."""
    if seed is None:
        return
...
def main(model_name = None):
    ...
    set_random_seeds(settings.get('seed'))
    ...
    # Starting positions in the underlying for the learned and benchmark hedge.
    ...
    for i in range(num_episodes):
        ...
        while not done:
            # Periodically print a deterministic episode without exploration noise.
            ...
            if show_example:
                ...
                # Store the noisy action in replay memory, then map it to a hedge ratio.
                action = np.clip( 0.5 * (pred_action[0] + 1), 0, 1)
