...
def model_artifact_path(name):
    ...
    matches = sorted(MODEL_DIR.glob(f"*/{name}"))
    if len(matches) == 1:
        return matches[0]
    if not matches:
        raise FileNotFoundError(f"Could not find model artifact {name}.")

    match_list = ", ".join(str(path) for path in matches)
    raise FileNotFoundError(f"Ambiguous model artifact {name}: {match_list}.")


def model_output_path(name):
...
class StatePrepare():
    ...
    def load(self, model_name = None):
        ...
        self.scaler = joblib.load(model_artifact_path(model_name + '_scaler'))
    
    def save(self, model_name = None):
        if model_name == None:
            model_name = self.name
        joblib.dump(self.scaler, model_output_path(model_name + '_scaler'))
